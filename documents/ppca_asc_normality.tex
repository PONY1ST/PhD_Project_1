...
$(\bm{X}_1, \ldots, \bm{X}_n)$
follows the true distribution $P^n_{[\bm{W}_0, \sigma_0^2]}$,
where $Z$ is a random variable following $N_d(\bm{0}, \bm{G}^{-1})$
distribution.
\end{theorem}
...
\end{equation*}
Since $\abs{\bm{U}_n\bm{U}_n^T}^2 = 	\|\bm{U}_n\|_F^2 = \abs{\bm{h}_n}^2$,
...
The rest of the conclusions follows immediately.
\end{proof}
